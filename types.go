...
type QuoteData struct {
	Price  *float64 `mapstructure:"lp"` //价格
	Volume *float64 `mapstructure:"volume"`
	Bid    *float64 `mapstructure:"bid"` //TODO 这个要看一下是否还有
	Ask    *float64 `mapstructure:"ask"` //TODO 这个要看一下是否还有

	PriceTime *float64 `mapstructure:"lp_time"` //报价时间, 1712979679， 十位时间戳
	Chp       *float64 `mapstructure:"chp"`     //价格变化百分比，比如-1.2 则指的跌了-1.2%
	Ch        *float64 `mapstructure:"ch"`      //价格变化值，比如跌了20，则是-20
